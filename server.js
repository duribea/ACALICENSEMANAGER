  ...
  try {
    const result = await sql.query`
      SELECT 
        ClienteID,
        NombreCliente,
        FORMAT(FechaExpiracionLicencia, 'dd-MM-yyyy') as FechaExpiracionLicencia,
        VersionAnalytics,
        VersionConnector,
        VersionAdapter,
        FORMAT(FechaActualizacion, 'dd-MM-yyyy') as FechaActualizacion
      FROM Clientes
    `;
  ...
  try {
    const result = await sql.query`
      SELECT 
        ...
        sa.NombreSistema,
        c.VersionSistema,
        FORMAT(c.FechaInstalacionACA, 'dd-MM-yyyy') as FechaInstalacionACA,
        FORMAT(c.FechaTermino, 'dd-MM-yyyy') as FechaTermino
      FROM Centros c
